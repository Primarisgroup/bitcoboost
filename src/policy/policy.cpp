...
bool AreInputsStandard(const CTransaction& tx, const CCoinsViewCache& mapInputs)
{
    ...
    for (unsigned int i = 0; i < tx.vin.size(); i++) {
        ...
        // BitcoBoost: la versione witness 2 (PQ ibrido bb1z) e' una versione DEFINITA,
        // quindi spenderla e' standard per il relay (le altre versioni ignote restano non-standard).
        {
            int bb_wv = 0; std::vector<unsigned char> bb_wp;
            if (prev.scriptPubKey.IsWitnessProgram(bb_wv, bb_wp) && bb_wv == 2 && bb_wp.size() == 32) {
                continue;
            }
        }
