...
{
    // === BitcoBoost: soft-defaults per porte P2P/RPC per rete ===
    // Usa SoftSetArg così l'utente può sempre override da CLI o conf.
    const std::string net = Params().GetChainTypeString();
    if (net == "main") {
        gArgs.SoftSetArg("-port",    "38200");
        gArgs.SoftSetArg("-rpcport", "38201");
        gArgs.SoftSetArg("-rpcbind", "127.0.0.1");
    } else if (net == "test") {
        gArgs.SoftSetArg("-port",    "38210");
        gArgs.SoftSetArg("-rpcport", "38211");
    } else if (net == "regtest") {
        gArgs.SoftSetArg("-port",    "38220");
        gArgs.SoftSetArg("-rpcport", "38221");
    } else if (net == "signet") {
        gArgs.SoftSetArg("-port",    "38230");
        gArgs.SoftSetArg("-rpcport", "38231");
    }
    // === /BitcoBoost ===
