...
{
    // Bitcoboost: slow start del block reward per disincentivare mining mordi-e-fuggi
    // nei primi blocchi del lancio mainnet.
    if (nHeight <= 1000) return 1 * COIN;
    if (nHeight <= 2000) return 10 * COIN;

    // Halving standard a partire dal blocco 2001 in poi.
    // Convenzione: halving_interval e calcolato sui blocchi DOPO lo slow start.
...
// ============================================================================
// Bitcoboost: founder reward 5% per i primi 4 anni
// ============================================================================
namespace BBFounderReward {
    constexpr int START_HEIGHT = 2001;       // dopo lo slow start
    constexpr int END_HEIGHT = 210000;       // ~4 anni (primo halving)
    constexpr int PERCENT = 5;
    // Address placeholder. DA SOSTITUIRE prima della compilazione di lancio
...
CAmount GetFounderRewardForBlock(int nHeight, const Consensus::Params& consensusParams) {
    if (nHeight < BBFounderReward::START_HEIGHT) return 0;
                               ...
                               CCoinsViewCache& view, bool fJustCheck)
{
    ...
    // Bitcoboost: enforce founder reward 5% per i primi 4 anni (blocchi 2001-210000).
    // Il coinbase deve includere un output al founder address di valore esatto = 5% del subsidy.
    {
        CAmount expectedFounderReward = GetFounderRewardForBlock(pindex->nHeight, params.GetConsensus());
        if (expectedFounderReward > 0) {
            CTxDestination founderDest = DecodeDestination(BBFounderReward::GetFounderAddress());
            if (!IsValidDestination(founderDest)) {
                LogPrintf("ERROR: ConnectBlock(): founder address invalid (compile-time bug)\n");
                return state.Invalid(BlockValidationResult::BLOCK_CONSENSUS, "bad-cb-founder-config");
            }
            CScript founderScript = GetScriptForDestination(founderDest);
            ...
            if (!foundFounderOutput) {
                LogPrintf("ERROR: ConnectBlock(): coinbase missing founder reward (height=%d expected=%d to %s)\n",
                          pindex->nHeight, expectedFounderReward, BBFounderReward::GetFounderAddress());
                return state.Invalid(BlockValidationResult::BLOCK_CONSENSUS, "bad-cb-founder-reward");
            }
        }
    }
