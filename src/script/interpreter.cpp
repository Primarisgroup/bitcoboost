...
extern "C" {
#include <crypto/mldsa/api.h>
#include <crypto/mldsa/fips202.h>
}
...
template <class T>
bool GenericTransactionSignatureChecker<T>::CheckHybridSignature(const std::vector<unsigned char>& ecdsa_sig, const std::vector<unsigned char>& ecdsa_pub, const std::vector<unsigned char>& mldsa_sig, const std::vector<unsigned char>& mldsa_pub, const std::vector<unsigned char>& program) const
{
    // BitcoBoost quantum-safe hybrid spend (ECDSA + ML-DSA-44), witness v2.
    if (ecdsa_pub.size() != 33) return false;
    if (mldsa_pub.size() != 1312) return false;
    if (program.size() != 32) return false;
    // commitment: SHAKE256(ecdsa_pub || mldsa_pub) == program
    std::vector<unsigned char> keymat;
    keymat.reserve(ecdsa_pub.size() + mldsa_pub.size());
    keymat.insert(keymat.end(), ecdsa_pub.begin(), ecdsa_pub.end());
    keymat.insert(keymat.end(), mldsa_pub.begin(), mldsa_pub.end());
    unsigned char commit[32];
    shake256(commit, 32, keymat.data(), keymat.size());
    if (memcmp(commit, program.data(), 32) != 0) return false;
    // sighash (BIP143) over the v2 scriptPubKey, SIGHASH_ALL
    CScript scriptCode;
    scriptCode << OP_2;
    scriptCode << program;
    ...
    // ECDSA (strip trailing sighash-type byte, require SIGHASH_ALL)
    if (ecdsa_sig.empty() || ecdsa_sig.back() != SIGHASH_ALL) return false;
    std::vector<unsigned char> e(ecdsa_sig.begin(), ecdsa_sig.end() - 1);
    CPubKey epk(ecdsa_pub);
    if (!epk.IsValid()) return false;
    if (!VerifyECDSASignature(e, epk, sighash)) return false;
    // ML-DSA-44 with fixed context
    static const unsigned char CTX[] = "BitcoBoost-bb1z-v2";
    if (PQCLEAN_MLDSA44_CLEAN_crypto_sign_verify_ctx(mldsa_sig.data(), mldsa_sig.size(), sighash.begin(), 32, CTX, sizeof(CTX) - 1, mldsa_pub.data()) != 0) return false;
    return true;
}
...
static bool VerifyWitnessProgram(const CScriptWitness& witness, int witversion, const std::vector<unsigned char>& program, unsigned int flags, const BaseSignatureChecker& checker, ScriptError* serror, bool is_p2sh)
{
    ...
    if (witversion == 0) {
        ...
        // BitcoBoost quantum-safe hybrid (ECDSA + ML-DSA-44), enforced from genesis.
        // Witness: [ecdsa_sig, ecdsa_pub(33), mldsa_sig, mldsa_pub(1312)]
        if (stack.size() != 4) return set_error(serror, SCRIPT_ERR_WITNESS_PROGRAM_MISMATCH);
        if (!checker.CheckHybridSignature(stack[0], stack[1], stack[2], stack[3], program)) {
            return set_error(serror, SCRIPT_ERR_WITNESS_PROGRAM_MISMATCH);
        }
        return set_success(serror);
    } else {
