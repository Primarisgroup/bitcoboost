import sys, os, json, http.client, base64, struct, hashlib, time

# === CONFIG ===
DATADIR = r"D:\bitcoboost\bb_main_boost7"
RPC_HOST = "127.0.0.1"
RPC_PORT = 38211
PAYOUT_ADDRESS = "BQtaQLBGt5JycnqzCv2H3g1xgaX63oMr4C"  # se vuoi, sostituiscilo con un tuo indirizzo-wallet
...
def build_block(prevhash, prev_time, bits_hex, height, message=b"/BB/"):
    bits = int(bits_hex, 16)
    target = bits_to_target(bits)
    version = 1
    prev_le = bytes.fromhex(prevhash)[::-1]
    curtime = max(int(prev_time) + 1, int(time.time()))
    COIN = 100_000_000
    cb = make_coinbase_tx(50 * COIN, height=height, message=message)
    merkle = hash256(cb)                    # tx only -> merkle = dblsha256(cb)
    # NOTE: merkle in header in big-endian (come ha funzionato per block 1)
    ...
    return block, d[::-1].hex(), nonce

def main():
    N = int(sys.argv[1]) if len(sys.argv) > 1 else 120
    for i in range(N):
        tip = rpc("getbestblockhash")
        hdr = rpc("getblockheader", [tip, True])
        height_next = rpc("getblockcount") + 1
        block, bhash, nonce = build_block(
            prevhash=tip,
            prev_time=hdr["time"],
            bits_hex=hdr["bits"],
            height=height_next
        ...
        print(f"[BB] i={i+1}/{N} height={height_next} hash={bhash} nonce={nonce} res={res}")
        # piccolo respiro per non saturare la console
        # time.sleep(0.01)
