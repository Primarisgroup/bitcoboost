...
RPC_HOST = "127.0.0.1"
RPC_PORT = 38211
PAYOUT_ADDRESS = "BQtaQLBGt5JycnqzCv2H3g1xgaX63oMr4C"  # non usato per la tx utente; serve solo per la coinbase
...
def make_coinbase_tx(value_sats, height:int, message=b"/BB/"):
    tx  = le32(1)
    tx += varint(1)
    tx += b"\x00"*32 + b"\xff"*4
...
def merkle_root(tx_bytes_list):
    hashes = [hash256(b) for b in tx_bytes_list]
    if not hashes: return b"\x00"*32
    while len(hashes) > 1:
        if len(hashes) & 1:
            hashes.append(hashes[-1])
        nxt = []
        for i in range(0, len(hashes), 2):
            nxt.append(hash256(hashes[i] + hashes[i+1]))
        hashes = nxt
    return hashes[0]  # ⬅️ stesso orientamento che abbiamo usato per block 1

def main():
    tip = rpc("getbestblockhash")
    tiphdr = rpc("getblockheader", [tip, True])
    height_next = rpc("getblockcount") + 1
    prev_le = bytes.fromhex(tip)[::-1]
    bits_hex = tiphdr["bits"]; bits = int(bits_hex, 16)
    target = bits_to_target(bits)
    curtime = max(int(tiphdr["time"]) + 1, int(time.time()))

    # Raccogli le tx dal mempool (le includiamo così come sono):
    txids = rpc("getrawmempool")
    raw_txs = []
    for txid in txids:
        try:
            rawhex = rpc("getrawtransaction", [txid, False])
            raw_txs.append(bytes.fromhex(rawhex))
        except Exception:
            pass  # se una tx sparisce tra chiamate, la ignoriamo

    # Coinbase (50 BB; non reclamiamo le fee: under-claim è valido)
    coinbase = make_coinbase_tx(50*COIN, height_next)
    tx_bytes_list = [coinbase] + raw_txs

    merkle = merkle_root(tx_bytes_list)

    # NB: come per block 1, mettiamo il merkle **senza reverse** nell'header
    header = le32(1) + prev_le + merkle + le32(curtime) + le32(bits) + le32(0)
    ...
    block = header_final + varint(len(tx_bytes_list)) + b"".join(tx_bytes_list)

    print(f"[BB] mempool_tx_count={len(raw_txs)} (coinbase excluded)")
    print(f"[BB] height={height_next} bits={bits_hex} nonce={nonce} blockhash={hash256(header_final)[::-1].hex()}")
